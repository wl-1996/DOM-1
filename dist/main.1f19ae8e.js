...
var nodes = dom.empty(window.empty); //找到id为empty的元素，干掉它的儿子

console.log(nodes);
dom.attr(test, "title", "Hi,I am Frank"); //三个参数改属性

var title = dom.attr(test, "title"); //两个参数读属性

console.log("title: ".concat(title)); //打印出读到的test元素的属性
...
dom.off(test, "click", fn); //找到id为test的第一个元素并赋值给testDiv:

var testDiv = dom.find("#test")[0];
console.log(testDiv); //在testDiv2里找到类为.red的元素，并打印出来:
